...
//		} 
	...
	public void paint(Graphics g) {
		super.paint(g);
		Location randomDungeon = randomDungeon(dungeons);
		paintDungeon(randomDungeon, g);
		
		
	}
	
	public void paintDungeon(Location d, Graphics g) {
...
//	    this.add(new JLabel(/*get the text inside dungeon d*/), BorderLayout.NORTH);
	...
	public void paintStats() {
	    this.add(new JLabel("hp : " + character.getHP() + "\n AD: " 
	+ character.getAttackDamage() + "\n AP : " + character.getAbilityPower()), 
	    		BorderLayout.NORTH);
	}
	
	public Location randomDungeon(ArrayList<Location> d) {
		int randNum = (int)(d.size() * Math.random());
		return d.get(randNum);
	}
	
	public void run(Location[] dungeons, MainCharacter c) {
